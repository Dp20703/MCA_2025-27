...
select BranchName from branch where city like 'Nagpur';

-- B. Subqueries using ANY

-- 7. Display customers whose account balance is greater than ANY loan amount.
select Name from customer where CustID in (select CustId from account where 
balance > any (select amount from loan));
...
-- 10. Display customers whose loan amount is greater than ANY loan amount taken in the ‘AJNI’ branch.
SELECT Name FROM Customer
WHERE CustID IN (  SELECT CustID 
  FROM Loan 
  WHERE Amount > ANY (    SELECT Amount     FROM Loan     WHERE BranchName = 'AJNI'  ));

-- 11. Display customers whose account balance is greater than ANY account balance in ‘Nagpur’ branches
SELECT Name FROM Customer
WHERE CustID IN (  SELECT CustID 
  FROM Account   WHERE Balance > ANY (
    SELECT Balance     FROM Account 
    WHERE BranchName IN (      SELECT BranchName FROM Branch WHERE City = 'Nagpur'    )  ));
...
FROM Customer WHERE CustID IN (
  ...
  WHERE Amount > ANY (
    SELECT Balance     FROM Account 
    WHERE BranchName IN (     SELECT BranchName FROM Branch WHERE City = 'Pune')  ));
...
SELECT Name FROM Customer
WHERE CustID IN (
  SELECT CustID   FROM Account   WHERE Balance > ALL (SELECT Amount FROM Loan)
);

-- 14. Display customers whose loan amount is greater than ALL account balances.
SELECT Name FROM Customer
WHERE CustID IN (  SELECT CustID 
  FROM Loan   WHERE Amount > ALL (SELECT Balance FROM Account)
);

-- 15. Find customers whose account balance is less than ALL loan amounts.
SELECT Name FROM Customer WHERE CustID IN (
  SELECT CustID 
  FROM Account   WHERE Balance < ALL (SELECT Amount FROM Loan)
);
...
SELECT Name FROM Customer WHERE CustID IN (  SELECT CustID   FROM Loan  
 WHERE Amount > ALL (    SELECT Amount     FROM Loan     WHERE BranchName IN (   
